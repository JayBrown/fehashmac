...
/* hvf 15.08.2016 corrected (?) non-byte alignment	
*/
...
HashReturn Keccak_HashUpdate(Keccak_HashInstance *instance, const BitSequence *data, DataLength databitlen)
{
    if ((databitlen % 8) == 0)
        ...
        if (ret == SUCCESS) {
			...
			/* alignment included - hvf 15.08.2016	*/
...
HashReturn Keccak_HashFinal(Keccak_HashInstance *instance, BitSequence *hashval)
{
    ...
    if (ret == SUCCESS) {
		...
		HashReturn ret2 = KeccakWidth1600_SpongeSqueeze(&instance->sponge, hashval, instance->fixedOutputLength/8);
	   return ret2;
	}
    else
        return ret;
